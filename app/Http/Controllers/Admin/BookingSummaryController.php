...
class BookingSummaryController extends Controller
{
   							    ...
   							    )
    {
        ...
        $this->module_view_folder           = "admin.booking_summary";
        $this->theme_color                  = theme_color();
        $this->admin_panel_slug             = config('app.project.admin_panel_slug');
        $this->module_url_path              = url(config('app.project.admin_panel_slug')."/booking_summary");		
    ...
    public function get_records(Request $request)
    {
        ...
        $json_result    = $json_result
                                    ->editColumn('build_action_btn',function($data) use ($current_context,$arr_current_user_access)
                                    {
                                        ...
                                        {
                                            $cancel_href =  url(config('app.project.admin_panel_slug')."/track_booking/cancel_trip_request").'/'.base64_encode($data->id);
