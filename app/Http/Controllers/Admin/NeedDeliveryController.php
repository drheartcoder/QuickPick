...
class NeedDeliveryController extends Controller
{
    use MultiActionTrait;

	public function __construct(NeedDeliveryModel $need_delivery,
								EmailService $email_service)  
	{
        $this->arr_view_data 		= [];
		$this->NeedDeliveryModel 	= $need_delivery;

        $this->BaseModel            = $this->NeedDeliveryModel;
        $this->EmailService 		= $email_service;

		$this->module_url_path 		= url(config('app.project.admin_panel_slug')."/need_delivery");
        $this->module_view_folder   = "admin.need_delivery";
        $this->module_title         = "Need Delivery";
        $this->theme_color          = theme_color();
	}

	public function index() 
	{	
		$arr_need_delivery = array();
		$obj_need_delivery = $this->BaseModel->orderBy('id','DESC')->get();

		if($obj_need_delivery != FALSE)
		{
			$arr_need_delivery = $obj_need_delivery->toArray();
		}

        
		$this->arr_view_data['arr_need_delivery'] = $arr_need_delivery;
