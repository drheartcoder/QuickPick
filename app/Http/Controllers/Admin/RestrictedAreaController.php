...
class RestrictedAreaController extends Controller
{
	...
	public function __construct(RestrictedAreaModel $restricted_area)
	{
		$this->RestrictedAreaModel      = $restricted_area;
		$this->BaseModel              = $this->RestrictedAreaModel;
		$this->arr_view_data          = [];
		$this->module_title           = "Restricted Area";
		$this->module_view_folder     = "admin.restricted_area";
		$this->theme_color            = theme_color();
		$this->admin_panel_slug       = config('app.project.admin_panel_slug');
		$this->module_url_path        = url(config('app.project.admin_panel_slug')."/restricted_area");
	}
	...
	public function index(Request $request)
	{
		...
		$obj_data = $this->RestrictedAreaModel
								->get();
		$user_type = "driver";
	...
	public function view_map($enc_id = FALSE)
	{
		...
		else
		{
			...
			$obj_areas  = $this->RestrictedAreaModel
									->where('id',$id)
									->first();
			if($obj_areas)
			{
	...
	public function create_existing(Request $request)
	{
		...
		$obj_restricted_areas = $this->RestrictedAreaModel
									->groupBy('name')
									->get();
	...
	{ 
		...
		$check_existance = $this->RestrictedAreaModel
		...
		$status = $this->RestrictedAreaModel->create($arr_data);
		if($status)
		{
			$status ="success";
		}
	...
	public function update(Request $request)
	{
		...
		if($id!="")
		{
			$result = $this->RestrictedAreaModel
			->where('id',$id)
			->update($arr_data);
			if($result)
			{
