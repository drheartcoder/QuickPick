...
class AuthController extends Controller
{
    public function __construct(EmailService $mail_service)
    {
        ...
        $this->module_view_folder = "company.auth";
        $this->EmailService       = $mail_service;
        $this->admin_panel_login   = config('app.project.admin_panel_slug');
    ...
    public function process_login(Request $request)
    {
        ...
        if ($validator->fails()) 
        {
            return redirect(config('app.project.company_panel_slug').'/login')
    ...
    public function update_password(Request $request)
    {
        ...
        try 
        {
            ...
            {  
                ...
                if (!$hasher->check($newPassword, $user->password)) {
                    ...
                    Flash::error('New Password can not be same as old password');
                    return redirect()->back();
                }
    ...
    public function built_mail_data($email, $reminder_code)
    {
        ...
        if($user)
        {
            ...
            $reminder_url = '<p class="email-button"><a target="_blank" href=" '.URL::to($this->company_panel_slug.'/validate_admin_reset_password_link/'.base64_encode($arr_user['id']).'/'.base64_encode($reminder_code) ).'">Reset Password</a></p><br/>' ;
    ...
    public function reset_password(Request $request)
    {
        ...
        if ($reminder = Reminder::complete($user, $reminder_code, $password))
        {
          ...
          return redirect($this->company_panel_slug.'/login');
        }
        else
        {
          Flash::error('Reset Password Link Expired');
    ...
    {
        Sentinel::logout();
        return redirect(url($this->admin_panel_login));
    }
}
