...
class DriverController extends Controller
{
                                ...
                                )
    {
        ...
        $this->module_url_earning_paths     = url(config('app.project.company_panel_slug')."/driver/driver_earning/");
    ...
    public function get_records(Request $request)
    {
        ...
        $json_result     = $json_result->editColumn('build_action_btn',function($data) use ($current_context)
                            ...
                            ->editColumn('build_action_btn',function($data) use ($current_context,$role,$arr_current_user_access)
                            {
                                ...
                                {
                                    $vehicle_view_href =  url(config('app.project.company_panel_slug')."/vehicle").'/view/'.base64_encode($data->vehicle_id);
                                }
                                ...
                                // $build_fair_charge_action = '<a class="btn btn-circle btn-to-success btn-bordered btn-fill show-tooltip review-cars" href="'.$fair_charge_href.'" title="View Fair Charge Request"><i class="fa fa-exchange"></i></a>';
    ...
    {     
        ...
        $obj_user = DB::table($user_details)
                                ...
                                ->where($prefixed_user_details.'.company_id',$this->company_id)
                                ->where($prefixed_user_details.'.is_company_driver','1')
    ...
    public function store(Request $request)
    {
        ...
        $arr_data['company_id']        = $arr_get_data['id'];
        $arr_data['is_active']         = 1;
        $arr_data['is_company_driver'] = 1;
        ...
        if($obj_user)
        {
            ...
            $arr_notification_data_company = $this->built_notification_data_company($arr_data); 
    ...
    public function view($enc_id)
    {
        ...
        $obj_user = $this->UserModel
                                ->select('id','first_name','last_name','driving_license','profile_image','email','mobile_no','country_code','password','country_name','state_name','city_name','address','post_code')
                                ->where('id',$enc_id)
                                ->first();
        if($obj_user)
        {
    ...
    public function edit($enc_id)
    {
        ...
        $obj_user = $this->UserModel
                                ...
                                ->select('id','first_name','last_name','driving_license','profile_image','email','mobile_no','country_code','password','country_name','state_name','city_name','address','post_code','latitude','longitude')
                                ->where('id',$id)
                                ->first();
        if($obj_user)
        {
    ...
    public function update(Request $request)
    {
        ...
        if($email != '') 
        {
            ...
            if($is_email_duplicate>0)
            {
                Flash::error('Driver with this email address already exists.');
                return redirect()->back()->withInput($request->all());
            }
        }
        ...
        if($is_mobile_no_duplicate>0)
        {
            Flash::error('Driver with this mobile number already exists.');
            return redirect()->back()->withInput($request->all());
        }
        ...
        //$arr_update['company_name'] = $arr_get_data['company_name'];
        $arr_update['company_name'] = $company_name;
        $arr_update['company_id']   = $arr_get_data['id'];
    ...
    private function built_notification_data_info($arr_data,$type)
    {
        ...
        if(isset($arr_data) && sizeof($arr_data)>0)
        {
          if(isset($type) && $type == 'REGISTER')
          {
            ...
            $arr_notification['title']             = $full_name.' register as a Driver on Quickpick under Company'.' '.$this->company_name;
            $arr_notification['view_url']          = $this->module_url_path;
            $arr_notification['view_url']          = '/'.config('app.project.admin_panel_slug')."/driver?driver_type=company"; // $this->module_url_path;
         }
    ...
    private function built_notification_data_company($arr_data)
    {
        ...
        if(isset($arr_data) && sizeof($arr_data)>0)
        {
            ...
            $arr_notification['view_url']          = '/'.config('app.project.company_panel_slug')."/driver"; // $this->module_url_path;
        }
        return $arr_notification;
    }
    ...
    public function fair_charge_request($enc_id)
    {
        ...
        $this->arr_view_data['page_title']                   = str_singular($this->module_title)." Fair Charges";
        $this->arr_view_data['module_title']                 = str_plural($this->module_title)." Fair Charges";
    ...
    public function change_request_status($enc_id,$type)
    {
        ...
        if($obj_data)
        {
            ...
            if($update_status == 'APPROVE' || $update_status == 'REJECT')
            {
                ...
                $arr_data['user_type']         = 'DRIVER';
                $arr_data['notification_type'] = 'Fare Charge Request ';
                                    
                $title = '';
                if($update_status == 'APPROVE')
                {
                    $title = 'Fare Charge request successfully accepted by Admin.';
                }
                else if($update_status == 'REJECT')
                {
                    $title = 'Fare Charge request rejected by Admin.';   
                }

                $arr_data['title']             = $title;
                $arr_data['view_url']          = url(config('app.project.admin_panel_slug')."/driver/change_request_status").'/'.base64_encode($driver_id);
    ...
    public function deposit_receipt($enc_id)
    {
        ...
        $obj_company_deposit_money = $this->DepositMoneyModel
                                                ...
                                                ->where('to_user_type','COMPANY_DRIVER')
                                                ->orderBy('id','DESC')
                                                ->get();
        ...
        $arr_driver_balance_information = $this->get_driver_balance_information($driver_id);
        ...
        $this->arr_view_data['arr_company_deposit_money']      = $arr_company_deposit_money;
    ...
    public function make_payment(Request $request)
    {
        ...
        $arr_data                   = [];
        $arr_data['from_user_id']   = $this->company_id;
        ...
        $arr_data['from_user_type'] = 'COMPANY';
        $arr_data['to_user_type']   = 'COMPANY_DRIVER';
    ...
    public function get_earning_records(Request $request)
    {
        ...
        $current_context = $this;
                ...
                $json_result    = $json_result
                                    ->editColumn('fare_charge',function($data) use ($current_context,$arr_current_user_access)
                                    {
                                        $fare_charge = 0;
                                    ...
                                    ->editColumn('distance',function($data) use ($current_context,$arr_current_user_access)
                                    {                 
                                        $distance = isset($data->distance) ? number_format($data->distance,2) : 0;
                                        $distance = $distance.' <strong>Miles</strong>';
                                        return  $distance;                                        
                                    })
                                    ->editColumn('total_amount',function($data) use ($current_context,$arr_current_user_access)
                                    {                 
                                    ...
                                    ->editColumn('applied_promo_code_charge',function($data) use ($current_context,$arr_current_user_access)
                                    {                 
                                    ...
                                    ->editColumn('total_charge',function($data) use ($current_context,$arr_current_user_access)
                                    {                 
                                    ...
                                    ->editColumn('driver_amount',function($data) use ($current_context,$arr_current_user_access)
                                    {     
                                        $arr_booking = [];
                                        if($data)
                                        {
                                    ...
                                    ->editColumn('admin_amount',function($data) use ($current_context,$arr_current_user_access)
                                    { 
                                        $arr_booking = [];
                                        if($data)
                                        {
    ...
    private function get_driver_balance_information($driver_id)
    {
