...
class ValidateAreaService
{
    ...
    public function check_validate_assigned_area($arr_assigned_area_data)
    {

        $pickup_lat    = isset($arr_assigned_area_data['pickup_lat']) ? $arr_assigned_area_data['pickup_lat'] :'';
        $pickup_lng    = isset($arr_assigned_area_data['pickup_lng']) ? $arr_assigned_area_data['pickup_lng'] :'';

        $drop_lat    = isset($arr_assigned_area_data['drop_lat']) ? $arr_assigned_area_data['drop_lat'] :'';
        $drop_lng    = isset($arr_assigned_area_data['drop_lng']) ? $arr_assigned_area_data['drop_lng'] :'';

        $assigned_area_cnt = 0;
        
        if(($pickup_lat!=''&& $pickup_lng!='') && ($drop_lat!=''&& $drop_lng!='')) 
        {
            $arr_pick_up_lat_lng = "$pickup_lat $pickup_lng";
            ...
            $arr_assigned_area = $this->get_all_assigned_area();

            if(isset($arr_assigned_area) && sizeof($arr_assigned_area)>0)
            {
                foreach ($arr_assigned_area as $zone)
                { 
    ...
    private function get_all_assigned_area()
    {
        $arr_assigned_area = [];

        $obj_assigned_area = $this->AssignedAreaModel
                                            ->select('id','co-ordinates')
                                            ->where('is_active','1')
                                            ->get();
        
        if($obj_assigned_area)
        {
            $arr_assigned_area = $obj_assigned_area->toArray();
        }

        return $arr_assigned_area;
    }
     ...
     private function get_all_restricted_area()
    {
        ...
        $obj_restricted_area = $this->RestrictedAreaModel
                                            ->select('id','co-ordinates')
                                            ->where('is_active','1')
                                            ->get();
