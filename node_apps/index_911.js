...
io.on('connection', function (socket) {
	...
	*/
	...
	socket.on('track_all_vehicles', function (data) {
		...
		//     if (value != undefined) {
		//       if (value.rider_id != undefined) {
		//         if (String(value.rider_id) == String(data.rider_id) && String(value.service_id) == String(data.service_id)) {
		//           // arr_user_socket.splice(index, 1);
		//           // arr_user_socket[index].id = value.id;
		//           if(arr_ongoing_rides.length > 0) {
		//             arr_ongoing_rides.forEach(function (inner_value, index) {
		//               if (inner_value != undefined) {
		//                 if (inner_value.rider_id != undefined) {
		...
		//                        arr_user_socket[index].id = data.id;            
		//                        console.log('test_ongoing_ride emitted : '+ data.id);
		//                     }
		//                   }
		//                 }
		//               }
		...
		//         }
		//         else if(String(value.rider_id) == String(data.rider_id)){
		//           arr_user_socket.splice(index, 1);
