...
<script type="text/javascript">
    ...
    function track_available_vehicles() {
        $.ajax({
                url:MODULE_URL_PATH+'/track_current_booking?status=IN_TRANSIT',
                type:'GET',
                dataType:'json',
                success:function(response)
                {
                    if(response.status == 'success'){
                        ...
                        console.log('arr_driver_details',arr_driver_details);
    ...
    function show_on_map_available_driver(obj_available_driver){
        ...
        if(obj_available_driver!=undefined && obj_available_driver.length>0){
            ...
            $.each(obj_available_driver,function(obj_index,obj_value){
                ...
                if(obj_value.booking_status!=undefined && obj_value.booking_status == 'IN_TRANSIT'){
                                    ...
                                    // console.log("map_markers ---> ",inner_value, inner_index,obj_value);
                                        if (inner_value != undefined) {
                                            if (inner_value['driver_id'] != undefined) {
                                                if (inner_value['driver_id'] == obj_value.driver_id && obj_value.booking_status == 'IN_TRANSIT') {
                ...
                else 
                {
                    ...
                    if (markers.length > 0) {
                        markers.forEach(function (value, index) {
                            if (value != undefined) {
                                if (value['driver_id'] != undefined) {
                                    if (value['driver_id'] == obj_value.driver_id && (obj_value.booking_status == 'TO_BE_PICKED' || obj_value.booking_status == 'COMPLETED')) {
                                        markers.splice(index, 1);
                                    }
                                }
                            }
    ...
    function SetMarker(data) {
        ...
        if(data.booking_status!=undefined && data.booking_status == 'IN_TRANSIT'){
            ...
            google.maps.event.addListener(marker, 'click', function() {
                ...
                var html = '<div class="modal-content map-driver-details-popup">'+
    ...
    function genrate_driver_html(){
        ...
        if(arr_driver_details.length>0){
            arr_driver_details.forEach(function (value, index) {
                ...
                if(value.booking_status == 'IN_TRANSIT'){
                    var booking_master_id     = btoa(parseInt(value.booking_master_id));
                    var booking_status       = btoa(value.booking_status);
                    view_url = BASE_URL+'/admin/track_booking/view?enc_id='+booking_master_id+'&status='+booking_status+'&curr_page=track_booking';
                }
                ...
                generated_html +=   '<li data-id="'+index+'">'+
                                       '<div class="avatar-outr"> <div class="avatar-img"><img src="'+BASE_URL+'/uploads/default-profile.png" alt="" /></div>'+
                                        ...
                                        '</div>'+
                                        '<div class="avatar-content pull-left extra-class" data-user-type="driver">'+
